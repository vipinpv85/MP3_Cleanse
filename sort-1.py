...
def get_functional():
  ...
  for val in open(currentDir + os.sep + list1, "rb" ).readlines():
    temphold = re.split(" ", val.strip(), 1)
    if (temphold[1].startswith("-")):
      temphold[1] = temphold[1].split("-",1)[1].strip()
    print "................ " + str(temphold)
    content2.update( {temphold[1]:temphold[0]} )
      
  print "DEBUG : " + str(content2)
  print "DEBUG : got contents of new torrent!"
...
if __name__ == '__main__':
  ...
  if (tempval == 4):
    ...
    try:
      ...
      print "\n Folder exist for  songs list path : " + str(songlistpath)

    except OSError:
      print "\n Folder does not exist for  songs list path : " + str(songlistpath)
